...
function encrypt() {
  ...
  for (let i = 0; i < encryptText.length; i++) {
    switch (encryptText[i]) {
      case "a":
        modifiedString += encryptText[i].replace(/a/g, "ai");
        break;
      case "e":
        modifiedString += encryptText[i].replace(/e/g, "enter");
        break;
      case "i":
        modifiedString += encryptText[i].replace(/i/g, "imes");
        break;
      case "o":
        modifiedString += encryptText[i].replace(/o/g, "ober");
        break;
      case "u":
        modifiedString += encryptText[i].replace(/u/g, "ufat");
        break;
      default:
        modifiedString += encryptText[i];
    }
  }
